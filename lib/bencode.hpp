...
template <template <typename...> typename T> struct variant_traits;

#define BENCODE_MAP_PROXY_FN_1(name, specs)                                    \
  template <typename T> decltype(auto) name(T &&t) specs {                     \
...
// incomplete types.
template <typename Key, typename Value> class map_proxy {
public:
  ...
  template <typename K> const mapped_type &at(K &&k) const {
    return proxy_->at(std::forward<K>(k));
  }
...
template <typename T> struct variant_traits_for;
...
template <> struct variant_traits<std::variant> {
  ...
  template <typename Variant> inline static auto index(const Variant &variant) {
...
template <> struct variant_traits<boost::variant> {
  template <typename Visitor, typename... Variants>
...
template <typename T> inline constexpr bool is_view_v = is_view<T>::value;
...
template <typename String> class str_reader {
public:
...
template <> class str_reader<std::string_view> {
public:
  template <typename Iter, typename Size>
...
template <typename Data> inline Data basic_decode(const char *s) {
...
template <typename Data> inline Data basic_decode_some(const char *&s) {
...
template <typename... T> inline data decode(T &&...t) {
  return basic_decode<data>(std::forward<T>(t)...);
}

template <typename... T> inline data decode_some(T &&...t) {
  return basic_decode_some<data>(std::forward<T>(t)...);
}

template <typename... T> inline data_view decode_view(T &&...t) {
  return basic_decode<data_view>(std::forward<T>(t)...);
}

template <typename... T> inline data_view decode_view_some(T &&...t) {
  return basic_decode_some<data_view>(std::forward<T>(t)...);
}

#ifdef BENCODE_HAS_BOOST
template <typename... T> inline boost_data boost_decode(T &&...t) {
  return basic_decode<boost_data>(std::forward<T>(t)...);
}

template <typename... T> inline boost_data boost_decode_some(T &&...t) {
  return basic_decode_some<boost_data>(std::forward<T>(t)...);
}

template <typename... T> inline boost_data_view boost_decode_view(T &&...t) {
...
public:
  ...
  template <typename T> inline list_encoder &add(T &&value);
...
template <typename T> void write_integer(std::ostream &os, T value) {
...
namespace detail {
template <typename T> inline list_encoder &list_encoder::add(T &&value) {
...
template <typename T> std::string encode(T &&t) {
  std::stringstream ss;
