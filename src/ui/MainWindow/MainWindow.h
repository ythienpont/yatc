...
private:
  ...
  std::unique_ptr<TorrentClient> torrent_client_; /**< Unique pointer to the TorrentClient instance. */
  std::thread torrent_client_thread_; /**< Thread for running the TorrentClient. */
  std::vector<GtkWidget *> drawing_areas_; /**< Vector of pointers to drawing area widgets. */
   ...
   * @param user_data User data passed to the callback.
   */
  static void file_open_response(GtkNativeDialog *dialog, int response_id, gpointer user_data);
};
